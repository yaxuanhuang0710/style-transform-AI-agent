...
const Project = () => {
  ...
  useEffect(() => {
    if (projectId !== 'new') {
      setLoading(true);
      fetch(`http://3.80.75.241:8000/project/show_detail/${projectId}`)
        .then(res => res.json())
        .then(data => {
          setProjectData(data);
          setLoading(false);
  ...
  const handleGenerateContent = () => {
    setLoading(true);
    fetch('http://3.80.75.241:8000/chatbot/chat_with_gpt', {
      method: 'POST',
  ...
  const handleSaveContent = async (content) => {
    ...
    //create Project
    try {
      const response = await fetch('http://3.80.75.241:8000/project/create', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
    ...
    try {
      ...
      const response = await fetch(`http://3.80.75.241:8000/project/update_project/${projectId}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({...projectData})
      });
